...
func testTXTRegistryNew(t *testing.T) {
	...
	_, err := NewTXTRegistry(p, "txt", "", "", time.Hour, "", []string{}, []string{}, false, nil, false, false, "")
	require.Error(t, err)

	_, err = NewTXTRegistry(p, "", "txt", "", time.Hour, "", []string{}, []string{}, false, nil, false, false, "")
	require.Error(t, err)

	r, err := NewTXTRegistry(p, "txt", "", "owner", time.Hour, "", []string{}, []string{}, false, nil, false, false, "")
	...
	_, err = NewTXTRegistry(p, "txt", "txt", "owner", time.Hour, "", []string{}, []string{}, false, nil, false, false, "")
	require.Error(t, err)
	...
	_, err = NewTXTRegistry(p, "", "", "owner", time.Hour, "", []string{}, []string{}, false, nil, false, false, "")
	require.NoError(t, err)

	_, err = NewTXTRegistry(p, "", "", "owner", time.Hour, "", []string{}, []string{}, false, aesKey, false, false, "")
	require.NoError(t, err)

	_, err = NewTXTRegistry(p, "", "", "owner", time.Hour, "", []string{}, []string{}, true, nil, false, false, "")
	require.Error(t, err)

	r, err = NewTXTRegistry(p, "", "", "owner", time.Hour, "", []string{}, []string{}, true, aesKey, false, false, "")
	require.NoError(t, err)
...
func testTXTRegistryRecordsPrefixed(t *testing.T) {
	...
	r, _ := NewTXTRegistry(p, "txt.", "", "owner", time.Hour, "wc", []string{}, []string{}, false, nil, false, false, "")
...
func testTXTRegistryRecordsSuffixed(t *testing.T) {
	...
	r, _ := NewTXTRegistry(p, "", "-txt", "owner", time.Hour, "", []string{}, []string{}, false, nil, false, false, "")
	records, _ := r.Records(ctx)
...
func testTXTRegistryRecordsPrefixedTemplated(t *testing.T) {
	...
	r, _ := NewTXTRegistry(p, "txt-%{record_type}.", "", "owner", time.Hour, "wc", []string{}, []string{}, false, nil, false, false, "")
	records, _ := r.Records(ctx)
...
func testTXTRegistryApplyChangesWithTemplatedPrefix(t *testing.T) {
	...
	r, _ := NewTXTRegistry(p, "prefix%{record_type}.", "", "owner", time.Hour, "", []string{}, []string{}, false, nil, false, false, "")
	changes := &plan.Changes{
		Create: []*endpoint.Endpoint{
			newEndpointWithOwnerResource("new-record-1.test-zone.example.org", "new-loadbalancer-1.lb.com", endpoint.RecordTypeCNAME, "", "ingress/default/my-ingress"),
		},
...
func testTXTRegistryApplyChangesWithTemplatedSuffix(t *testing.T) {
	...
	r, _ := NewTXTRegistry(p, "", "-%{record_type}suffix", "owner", time.Hour, "", []string{}, []string{}, false, nil, false, false, "")
	changes := &plan.Changes{
		Create: []*endpoint.Endpoint{
			newEndpointWithOwnerResource("new-record-1.test-zone.example.org", "new-loadbalancer-1.lb.com", endpoint.RecordTypeCNAME, "", "ingress/default/my-ingress"),
		},
...
func testTXTRegistryApplyChangesWithSuffix(t *testing.T) {
	...
	r, _ := NewTXTRegistry(p, "", "-txt", "owner", time.Hour, "wildcard", []string{}, []string{}, false, nil, false, false, "")
...
func testTXTRegistryMissingRecordsNoPrefix(t *testing.T) {
	...
	r, _ := NewTXTRegistry(p, "", "", "owner", time.Hour, "wc", []string{endpoint.RecordTypeCNAME, endpoint.RecordTypeA, endpoint.RecordTypeNS}, []string{}, false, nil, false, false, "")
	records, _ := r.Records(ctx)
...
func testTXTRegistryMissingRecordsWithPrefix(t *testing.T) {
	...
	r, _ := NewTXTRegistry(p, "txt.", "", "owner", time.Hour, "wc", []string{endpoint.RecordTypeCNAME, endpoint.RecordTypeA, endpoint.RecordTypeNS, endpoint.RecordTypeTXT}, []string{}, false, nil, false, false, "")
	records, _ := r.Records(ctx)
...
func TestTXTRegistryApplyChangesEncrypt(t *testing.T) {
	...
	r, _ := NewTXTRegistry(p, "txt.", "", "owner", time.Hour, "", []string{}, []string{}, true, []byte("12345678901234567890123456789012"), false, false, "")
	records, _ := r.Records(ctx)
	changes := &plan.Changes{
		Delete: records,
	}
...
func TestMultiClusterDifferentRecordTypeOwnership(t *testing.T) {
	...
	r, _ := NewTXTRegistry(p, "_owner.", "", "bar", time.Hour, "", []string{}, []string{}, false, nil, false, false, "")
	records, _ := r.Records(ctx)
...
func TestNewTXTRegistryWithNewFormatOnly(t *testing.T) {
	...
	r, err = NewTXTRegistry(p, "txt", "", "owner", time.Hour, "", []string{}, []string{}, false, nil, true, false, "")
	require.NoError(t, err)
...
func TestApplyChangesWithNewFormatOnly(t *testing.T) {
	...
	r, _ := NewTXTRegistry(p, "", "", "owner", time.Hour, "", []string{}, []string{}, false, nil, true, false, "")
